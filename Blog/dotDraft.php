...
<!-- These links attach icons for iOS devices to use as the favicon image. -->
	...
	<title>John Wickham.Draft</title>
	<meta charset="UTF-8">
...
<body onload="viewDidLoad();">
    ...
    <?php
		...
		if (isset($_GET["post"])) {
			...
			$draftsQuery = query("SELECT * FROM drafts WHERE identifier='$identifier'");
			if (isset($draftsQuery["result"]) && count($draftsQuery["result"]) > 0) {
					
				$aDraft = $draftsQuery["result"][0];
		
				?>
					<div style="width: 100vw; height: 100px; text-align: left; width: auto; margin-left: <?php echo($mobile ? "9" : "15"); ?>%;">
						<a href="https://wjwickham.com/blog/dotDraft.php"><img src="Images/dotDraft_Small.png" width="34" height="34"/></a>
				    </div>			
				<?php
		
				$newRow = '<div class="UIPostContainer" id="post' . $aDraft["identifier"] . '">';
				$newRow .= '<div class="UIDetailsContainer">';
				$newRow .= '<div class="UIDetailsContainer">';
				$newRow .= '<h2>' . $aDraft["title"] . '</h2>';
				
				$date = strtotime($aDraft["date"]);
				$newRow .= '<h3>' . date("D", $date) . ". " . date("j F Y", $date) . '</h3>';
				
				//Tag processing
				
				$body = str_replace("\\", "", $aDraft["body"]);
				$body = str_replace("WICK-PERCENT-ESCAPE", "%", $body);
				
				$newRow .= '</div>';
				$newRow .= '<h4>' . $aDraft["subtitle"] . '</h4>';
				$newRow .= '<p1>' . $body . '</p1></div></div>';
				echo($newRow);
			}	
			else {
				echo(".Draft couldn't find the post you're looking for.");
			}	
...
<div class="page-header">
					<img class="satatic" id="UINavigationEmblemUnder" src="Images/Emblem.png" width="34" height="34"/>
					<a href="https://wjwickham.com/"><img class="static" src="Images/Emblem_Invert.png" width="40" height="40" id="UINavigationEmblemOver"/></a>
		  	      <div class="page-headerSeparator"></div>
		  	  </div>
-->
		  	  
		  	  <img src="Images/dotDraft_Medium.png" id="headerImage" onclick="if(window.location.hash){window.location.replace('https://wjwickham.com/blog');}"/>
		  	  <div id="imageAccent"></div>
		  	  
		  	  <h1>The latest drafts from the .Write blog</h1>
		  	  
		  	  <div class="UISeparatorView"></div>
		  	  
		  	  <?php
			
				$draftsQuery = query("SELECT * FROM drafts");
		
				$drafts = array_reverse($draftsQuery["result"]);
			
				foreach ($drafts as $key => $aDraft) { 
					
					$body = str_replace("\\", "", $aDraft["body"]);
					$body = str_replace("WICK-PERCENT-ESCAPE", "%", $body);
					$previewString = (strlen($body) > 13) ? substr($body, 0, 200).'…' : $body;
					
					$newRow = '<div class="UIPreviewContainer" onclick="location.assign(\'https://wjwickham.com/blog/dotDraft.php?post=' . $aDraft["identifier"] . '\');" id="post' . $aDraft["identifier"] . '">';
					$newRow .= '<div class="UIDetailsContainer">';
					$newRow .= '<h2>' . $aDraft["title"] . '</h2>';
				...
				?>
					<footer id="UIPageFooter" style="border-top: 1px solid rgba(0,0,0,0.15);">
						<p style="color: rgba(0,0,0,0.30); font-weight: 300; font-family: -apple-system, 'Helvetica', 'Arial', sans-serif; font-size: 15px;">&copy; 2015 John Wickham.<br>These posts are considered unreleased and incomplete; keep in mind that they're likely to change before being published.</p>
				    </footer>	
				<?php
